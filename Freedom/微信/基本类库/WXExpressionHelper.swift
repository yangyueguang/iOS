...
class WXExpressionHelper: NSObject {
    ...
    let IEXPRESSION_NEW_URL = IEXPRESSION_HOST_URL + ("expre/listBy.do?pageNumber=%ld&status=Y&status1=B")
    let IEXPRESSION_BANNER_URL = IEXPRESSION_HOST_URL + ("advertisement/getAll.do?status=on")
    let IEXPRESSION_PUBLIC_URL = IEXPRESSION_HOST_URL + ("expre/listBy.do?pageNumber=%ld&status=Y&status1=B&count=yes")
    let IEXPRESSION_SEARCH_URL = IEXPRESSION_HOST_URL + ("expre/listBy.do?pageNumber=1&status=Y&eName=%@&seach=yes")
    let IEXPRESSION_DETAIL_URL = IEXPRESSION_HOST_URL + ("expre/getByeId.do?pageNumber=%ld&eId=%@")
        ...
        for i in 0...group?.data.count ?? 0 {
            downloadGroup.async(group: downloadQueue, execute: {
                ...
                if i == group?.data.count {
                    ...
                    if data == nil {
                        if let anID = emoji?.emojiID {
                            urlString = "http://123.57.155.230:8080/ibiaoqing/admin/expre/downloadsuo.do?pId=\(anID)"
                        }
                        if let aString = URL(string: urlString ?? "") {
                            data = Data(contentsOf: aString)
                        }
