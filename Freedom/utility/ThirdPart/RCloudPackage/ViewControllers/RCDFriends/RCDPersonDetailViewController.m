...
- (void)viewDidLoad {
  ...
  if (portraitUri.length == 0) {
    ...
    [[UIView alloc] initWithFrame:CGRectMake(0, 0, 100, 100)];
      ...
      NSString *firstLetter = [self.friendInfo.name pinyinFirstLetter];
    ...
    [self.ivAva sd_setImageWithURL:[NSURL URLWithString:portraitUri] placeholderImage:[UIImage imageNamed:@"icon_person"]];
  }
  __weak RCDPersonDetailViewController *weakSelf = self;
  [[RCIMClient sharedRCIMClient] getBlacklistStatus:self.friendInfo.userId success:^(int bizStatus) {
...
- (void)rightBarButtonItemClicked:(id)sender {
    NSString *btnTitle = @"取消黑名单";
  if (!self.inBlackList) {
      btnTitle = @"加入黑名单";
  }
    [self showAlerWithtitle:nil message:nil style:UIAlertControllerStyleActionSheet ac1:^UIAlertAction *{
        return [UIAlertAction actionWithTitle:btnTitle style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
- (void)setLayoutForFriend{
  ...
  UITapGestureRecognizer *clickRemarksView = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(gotoRemarksView:)];
  ...
  [self.view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|[_infoView(85)]" options:0 metrics:nil views:self.subViews]];
  [self.view addConstraints:[NSLayoutConstraint
...
- (void)setLayoutIsHaveRemarks:(BOOL)isHaveRemarks{
  ...
  [self.infoView addConstraints:[NSLayoutConstraint
                                 ...
                                 options:0
                                 metrics:nil  views:NSDictionaryOfVariableBindings(_phoneNumberLabel,_onlineStatusLabel)]];
...
-(void)setPhoneNumber {
   self.phoneNumberLabel.textColor = [FreedomTools colorWithRGBHex:0x999999];
    ...
    UITapGestureRecognizer *clickPhoneNumber = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(doCall:)];
