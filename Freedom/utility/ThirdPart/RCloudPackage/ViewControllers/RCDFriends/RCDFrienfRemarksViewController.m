...
if ([remarksStr isEqualToString:self.friendInfo.displayName]) {
    ...
    [self.hud show:YES];
    [RCDHTTPTOOL setFriendDisplayName:self.friendInfo.userId displayName:remarksStr complete:^(BOOL result) {
       if (result == YES) {
         self.rightBtn.userInteractionEnabled = YES;
         [self.hud hide: YES];
         self.friendInfo.displayName = remarksStr;
         [[RCDataBaseManager shareInstance] insertFriendToDB:self.friendInfo];
         RCUserInfo *user = [[RCUserInfo alloc] initWithUserId:self.friendInfo.userId name:remarksStr portrait:self.friendInfo.portraitUri];
