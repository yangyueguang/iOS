...
- (void)notifyWatchKitEvent:(NSString *)appEvent {
    if (self.isWatchAttached) {
        CFNotificationCenterPostNotification(CFNotificationCenterGetDarwinNotifyCenter(),(__bridge CFStringRef)appEvent, NULL, NULL, true);
    }
}
...
- (BOOL)handleWatchKitRequest {
  ...
  if ([query isEqualToString:WK_APP_COMMUNICATE_QUERY_UNREAD_COUNT]) {
    ...
    NSArray *messages = [[RCIMClient sharedRCIMClient] getHistoryMessages:[conversationType intValue] targetId:targetID oldestMessageId:[olderMsgId longValue] count:[count intValue]];
        ...
        dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
          for (RCUserInfo *userInfo in array) {
            ...
            if (![fileManager fileExistsAtPath:[currentUrl path]]) {
              UIImage *image = [UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:userInfo.portraitUri]]];
            ...
            if (conversationType.intValue == ConversationType_PUBLICSERVICE ||
              ...
              for (RCPublicServiceProfile *profile in publicServices) {
                if ([profile.publicServiceId isEqualToString:targetId]) {
                  UIImage *image = [UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:profile.portraitUrl]]];
                  NSLog(@"size is %f, %f", image.size.width, image.size.height);
                  if (image && [image size].width > 0) {
    ...
    NSString *targetID = [self getQueryParameter:WK_APP_COMMUNICATE_PARAMETER_TARGET_ID];
    NSNumber *conversationType = [self getQueryParameter:WK_APP_COMMUNICATE_PARAMETER_CONVERSATION_TYPE];
    BOOL result = [[RCIMClient sharedRCIMClient] clearMessagesUnreadStatus:[conversationType intValue] targetId:targetID];
    ...
    if ([self isLocalPath:imageUrl]) {
      ...
      [[RCIMClient sharedRCIMClient] downloadMediaFile:[conversationType intValue] targetId:targetId mediaType:MediaType_IMAGE mediaUrl:imageUrl progress:^(int progress) {
            NSLog(@"progress %d", progress);
          }success:^(NSString *mediaPath) {
            NSLog(@"[RongIMKit]:downloadMediaFile.mediaPath > %@, isMainThread > %d",mediaPath, [NSThread isMainThread]);
            NSString *toPath = @"loadedImage.tmp";
