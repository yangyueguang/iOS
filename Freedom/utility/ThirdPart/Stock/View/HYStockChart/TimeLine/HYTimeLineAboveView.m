...
//  JimuStockChartDemo
//  Created by jimubox on 15/5/8  Copyright (c) 2015年 jimubox. All rights reserved.
...
- (void)drawDashLine {
    ...
    /*
     ...
     CGContextSetFillColorWithColor(ctx, [UIColor colorWithWhite102].CGColor);
    ...
    CGContextSetFillColorWithColor(ctx, [UIColor colorWithWhite:102/255.0 alpha:1].CGColor);
...
- (CGRect)rectOfNSString:(NSString *)string attribute:(NSDictionary *)attribute {
    CGRect rect = [string boundingRectWithSize:CGSizeMake(MAXFLOAT, 0) options:NSStringDrawingTruncatesLastVisibleLine |NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:attribute context:nil];
    return rect;
}
@end
...
- (void)drawGridBackground:(CGContextRef)context
    ...
    if (self.centerViewType == HYStockChartCenterViewTypeTimeLine){
        //画中间的线
        [self drawline:context startPoint:CGPointMake((rect.size.width ) / 2,0) stopPoint:CGPointMake(rect.size.width / 2, HYStockChartKLineAboveViewMaxY) color:GridLineColor lineWidth:HYStockChartTimeLineGridWidth];
    }
}
#pragma mark - **************** 长按操作
...
-(UIView *)timeLabelView{
    if (!_timeLabelView) {
        ...
        [_timeLabelView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.width.left.equalTo(self); make.height.equalTo(@(HYStockChartTimeLineTimeLabelViewHeight));
        }];
...
-(NSArray *)private_convertTimeLineModlesToPositionModel{
    ...
    if (self.endPointShowEnabled){
        ...
        [self.breathingPointView showCircleAnimationLayerWithColor:TimeLineLineColor andScale:4.0f];
    }
...
-(CGFloat)private_oneDayTradeTimes{
    HYStockType stockType = [HYStockChartTool stockType];
    switch (stockType) {
        case HYStockTypeA:return 240;break;
        case HYStockTypeUSA:return 390;break;
...
- (UIView *)breathingPointView{
    if (_breathingPointView == nil){
        ...
        _breathingPointView.backgroundColor = TimeLineLineColor;
