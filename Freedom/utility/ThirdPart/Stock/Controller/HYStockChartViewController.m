...
- (void)viewDidLoad{
    [super viewDidLoad];
    self.stockChartView.backgroundColor = [UIColor whiteColor];
...
-(UIButton *)cancelBtn{
    if (!_cancelBtn) {
        ...
        [_cancelBtn setImage:[UIImage imageNamed:@"compose_photo_close"] forState:UIControlStateNormal];
...
-(void)setStockChartProfile{
    ...
    profileModel.stockType = HYStockTypeUSA;
...
-(void)event_timeLineRequestMethod{
    ...
    self.groupTimeLineModel.timeLineModels = [JMSTimeLineModel mj_objectArrayWithKeyValuesArray:datas];
...
-(void)event_fiveDaysTimeLineRequestMethod{
    ...
    NSDictionary *dict = [NSDictionary dictionaryWithContentsOfFile:path];
    NSArray *datas = [dict objectForKey:@"Bars"];
    self.fiveDaysModels = [JMSTimeLineModel mj_objectArrayWithKeyValuesArray:datas];
    [self.stockChartView reloadData];
}
...
-(void)event_daylyKLineRequestMethodWithStartDate:(NSDate *)startDate endDate:(NSDate *)endDate{
    ...
    NSDictionary *dict = [NSDictionary dictionaryWithContentsOfFile:path];
    self.dayKLineGroupModel = [JMSKLineGroupModel mj_objectWithKeyValues:dict];
    [self.stockChartView reloadData];
}
...
-(void)event_weeklyKLineRequestMethodWithStartDate:(NSDate *)startDate endDate:(NSDate *)endDate{
    ...
    NSDictionary *dict = [NSDictionary dictionaryWithContentsOfFile:path];
    self.weekKLineGroupModle = [JMSKLineGroupModel mj_objectWithKeyValues:dict];
    [self.stockChartView reloadData];
}
...
-(void)event_monthlyKLineRequestMethodWithStartDate:(NSDate *)startDate endDate:(NSDate *)endDate{
    ...
    NSDictionary *dict = [NSDictionary dictionaryWithContentsOfFile:path];
    self.monthKLineGroupModle = [JMSKLineGroupModel mj_objectWithKeyValues:dict];
    [self.stockChartView reloadData];
}
...
-(void)event_refreshData{
    ...
    switch (self.stockChartView.currentIndex) {
        case 0:[self event_timeLineRequestMethod];break;//时分线
        case 1:[self event_fiveDaysTimeLineRequestMethod];break;//5日线
        case 2:{
            self.dayKLineGroupModel.GlobalQuotes = nil;
            [self event_daylyKLineRequestMethodWithStartDate:nil endDate:nil];
        }break;//日K线
        case 3:[self.indicatorView stopAnimating];break;//周K
        case 4:[self.indicatorView stopAnimating];break;
        default:break;
    }
}
    ...
    switch (index) {
        case 0://时分
            if (self.groupTimeLineModel.timeLineModels.count > 0) {
                //先将jms转换成hy
                NSArray *jmsTimeLineDict = [JMSTimeLineModel mj_keyValuesArrayWithObjectArray:self.groupTimeLineModel.timeLineModels];
                NSArray *hyTimeLineModels = [HYTimeLineModel mj_objectArrayWithKeyValuesArray:jmsTimeLineDict];
        ...
        case 1://5日线
            if (self.fiveDaysModels.count > 0) {
                NSArray *jmsTimeLineDict = [JMSTimeLineModel mj_keyValuesArrayWithObjectArray:self.fiveDaysModels];
                NSArray *hyTimeLineModels = [HYTimeLineModel mj_objectArrayWithKeyValuesArray:jmsTimeLineDict];
                HYTimeLineGroupModel *hyTimeGroupModel = [HYTimeLineGroupModel new];
        ...
        case 2://日K线
            if (self.dayKLineGroupModel.GlobalQuotes > 0) {
                //先将JMS转换成HY
                NSArray *jmsKLineDict = [JMSKLineModel mj_keyValuesArrayWithObjectArray:self.dayKLineGroupModel.GlobalQuotes];
                NSArray *hyKLineModels = [HYKLineModel mj_objectArrayWithKeyValuesArray:jmsKLineDict];
                [self.indicatorView stopAnimating];
        ...
        case 3:
            ...
            if (self.weekKLineGroupModle.GlobalQuotes > 0) {
                //先将JMS转换成HY
                NSArray *jmsKLineDict = [JMSKLineModel mj_keyValuesArrayWithObjectArray:self.weekKLineGroupModle.GlobalQuotes];
                NSArray *hyKLineModels = [HYKLineModel mj_objectArrayWithKeyValuesArray:jmsKLineDict];
                [self.indicatorView stopAnimating];
                return hyKLineModels;
            }else{
        ...
        case 4:
            ...
            if (self.monthKLineGroupModle.GlobalQuotes > 0) {
                //先将JMS转换成HY
                NSArray *jmsKLineDict = [JMSKLineModel mj_keyValuesArrayWithObjectArray:self.monthKLineGroupModle.GlobalQuotes];
                NSArray *hyKLineModels = [HYKLineModel mj_objectArrayWithKeyValuesArray:jmsKLineDict];
                [self.indicatorView stopAnimating];
                return hyKLineModels;
            }else{
...
- (UIInterfaceOrientationMask)supportedInterfaceOrientations{
    return self.isFullScreen == YES ?  UIInterfaceOrientationMaskLandscapeRight : UIInterfaceOrientationMaskPortrait;
}
@end
