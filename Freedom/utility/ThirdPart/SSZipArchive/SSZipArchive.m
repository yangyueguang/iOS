...
- (BOOL)open {
	...
	_zip = cr_zipOpen([_path UTF8String], APPEND_STATUS_CREATE);
	return (NULL != _zip);
}
...
- (BOOL)writeFileAtPath:(NSString *)path withFileName:(NSString *)fileName {
    ...
    cr_zipOpenNewFileInZip(_zip, afileName, &zipInfo, NULL, 0, NULL, 0, NULL, Z_DEFLATED, Z_DEFAULT_COMPRESSION);
    ...
    while (!feof(input))
    {
		...
		cr_zipWriteInFileInZip(_zip, buffer, len);
	}

	cr_zipCloseFileInZip(_zip);
	free(buffer);
	fclose(input);
	return YES;
}
...
- (BOOL)writeData:(NSData *)data filename:(NSString *)filename {
	...
	cr_zipOpenNewFileInZip(_zip, [filename UTF8String], &zipInfo, NULL, 0, NULL, 0, NULL, Z_DEFLATED, Z_DEFAULT_COMPRESSION);

    cr_zipWriteInFileInZip(_zip, data.bytes, (unsigned int)data.length);
